function []=rcwa_data_phi_theta_sym_invgeom(ldnm,T_noskewx,T_noskewy,MS,Notch,height_grating_nm,thickness_coating_nm,MN,zeta,thetas,phi_sym)

%Note: You have only inverted the geometry for the diamond_notch case. Have yet to do that for the other geometries. -- 23 Nov 2018
...
%n1p7=[1.7258 1.7182 1.7155 1.7143 1.7048 1.7 1.6951 1.6941];

xn1p7=dlmread(resin_nk_file,'',2,0);
x1p7=xn1p7(:,1)*1e-9;
...
Ty=T_noskewy*1e-9/cosd(zeta);
	...
	switch zeta
		...
		case 30
			...
			[dpts, npts, tns, texture, ~]=diamond_notch(nxt,height_grating_nm,MSr,Notchr,thickness_coating_nm,dphys,ld);
...
if speciphi==0
		...
		[outdir,outdirld]=dir_names(ldnm,Tnm,MS,Notch,height_grating_nm,thickness_coating_nm,oprefix,osuffix,rdrfold);
		...
		outfename='test';
		if isempty(iptfile)
			[outfename,~]=dir_names(ldnm,Tnm,MS,Notch,height_grating_nm,thickness_coating_nm,oprefix,osuffix,'');
		end
...
switch length(phis)
	...
	case 1
		...
		parfor ii=1:length(thetas)
			...
			[Ru,Td,DEr_count,DEt_count,summe]=rezeta(W1,W2,Kx,Ky,q1m,k1zi,kNzi,tns,nxt,retainord,theta,psi,phi,zeta,k0);			
...
switch length(phis)
	...
	case 1
		...
		%save AK1blue_459_phi90 DEr pr DEt pt phis thetas MS Notch  retainord cx cy retnxy
	...
	otherwise
		...
		%save D06ortho_sym DEr pr DEt pt phis thetas MS Notch  retainord cx cy retnxy
...
function [outdir,outdirld]=dir_names(ldnm,Tnm,MS,Notch,height_grating_nm,thickness_coating_nm,oprefix,osuffix,rdrfold)
...
switch length(Tnm)
	...
	case 1
		...
		if fMS==MS 
			if fNotch==Notch
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(MS),'MS_',num2str(Notch),'N_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(MS),'MS_',num2str(fNotch),'p5N_',num2str(thickness_coating_nm),'T');
			end
			
		else
			if fNotch==Notch
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(fMS),'p5MS_',num2str(Notch),'N_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(fMS),'p5MS_',num2str(fNotch),'p5N_',num2str(thickness_coating_nm),'T');
			end
	...
	case 2
		...
		if fMS==MS 
			if fNotch==Notch
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(MS),'MS_',num2str(Notch),'N_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(MS),'MS_',num2str(fNotch),'p5N_',num2str(thickness_coating_nm),'T');
			end
			
		else
			if fNotch==Notch
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(fMS),'p5MS_',num2str(Notch),'N_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(fMS),'p5MS_',num2str(fNotch),'p5N_',num2str(thickness_coating_nm),'T');
			end
...
function [dpts, npts, tns, texture, layers]=diamond_notch(nxt,H,MS,Notch,T,dphys,ld)
