function []=rcwa_data_phi_theta_sym(ldnm,T_noskewx,T_noskewy,MS,Notch,height_grating_nm,thickness_coating_nm,MN,zeta,thetas,phi_sym)

tic

speciphi=isnumeric(phi_sym{1});

oprefix=phi_sym{2};
osuffix=phi_sym{3};
rdrfold=phi_sym{4};
iptfile=phi_sym{5};
resin_nk_file=phi_sym{6};
	...
	switch zeta
		case 0
            ...
            if isempty(iptfile)
			    ...
			    [dpts, npts, tns, texture, ~]=diamond_ortho_inputfile(nxt,height_grating_nm,iptfile,thickness_coating_nm,dphys,ld); 
                tzr=['Epsilon map taken from: ',iptfile];
                disp(tzr);
			end
...
if speciphi==0
		...
		[outdir,outdirld]=dir_names(ldnm,Tnm,MS,Notch,height_grating_nm,thickness_coating_nm,oprefix,osuffix,rdrfold,scross);
		
		if exist(outdirld,'dir')
			disp('It seems the destination folder already exists. Change the folder prefix or suffix and run again.');return;
		end
		
		waphi=phi_sym{1};
			switch length(waphi)
				case 23 %'tables_without_symmetry'			
					phis=[-180:180];
				otherwise %'tables_with_symmetry'
					make_sym=1;
					switch zeta
						case 0
							phis=0:90;
		...
		outfename='test';
		if isempty(iptfile)
			[outfename,~]=dir_names(ldnm,Tnm,MS,Notch,height_grating_nm,thickness_coating_nm,oprefix,osuffix,'',scross);
		end
		fname=strcat(outfename,'_',num2str(ldnm),'_','phitabs_','R_avg','.xls');
end
...
switch length(phis)
	...
	case 1
		...
		for ii=1:numel(snxy)
			...
			xlswrite(fname, snxy, tabname, 'B1');
			xlswrite(fname, pr1, tabname, 'A2');
		
		
		save(outfename, 'DEr', 'pr', 'DEt', 'pt', 'phis', 'thetas', 'MS', 'Notch',  'retainord', 'retnxy');
	...
	otherwise
		...
		raw_file='rcwa_tables.m';
		copyfile(raw_file,outdir);

		parentdir=pwd;
		

		for ii=1:length(retainord)
			ename=strcat('R_avg_',num2str(retnxy(ii,1)),'_',num2str(retnxy(ii,2)),'.csv');
			fname=strcat(parentdir,'\',outdirld,'\',ename);
			
			pr1=[thetas.' pr(:,:,ii)];	
			
			dlmwrite(fname,phis,'delimiter',',','coffset',1);
			dlmwrite(fname,pr1,'delimiter',',','-append');
            
            
            ename=strcat('R_PP_',num2str(retnxy(ii,1)),'_',num2str(retnxy(ii,2)),'.csv');
			fname=strcat(parentdir,'\',outdirld,'\',ename);
			
			prl=[thetas.' prPP(:,:,ii)];	
			
			dlmwrite(fname,phis,'delimiter',',','coffset',1);
			dlmwrite(fname,prl,'delimiter',',','-append');
            
            ename=strcat('R_PS_',num2str(retnxy(ii,1)),'_',num2str(retnxy(ii,2)),'.csv');
			fname=strcat(parentdir,'\',outdirld,'\',ename);
            ...
            ename=strcat('R_SP_',num2str(retnxy(ii,1)),'_',num2str(retnxy(ii,2)),'.csv');
			fname=strcat(parentdir,'\',outdirld,'\',ename);
            ...
            ename=strcat('R_SS_',num2str(retnxy(ii,1)),'_',num2str(retnxy(ii,2)),'.csv');
			fname=strcat(parentdir,'\',outdirld,'\',ename);
			...
			ename=strcat('T_avg_',num2str(retnxy(ii,1)),'_',num2str(retnxy(ii,2)),'.csv');
			fname=strcat(parentdir,'\',outdirld,'\',ename);
			
			pt1=[thetas.' pt(:,:,ii)];	
			
			dlmwrite(fname,phis,'delimiter',',','coffset',1);
			dlmwrite(fname,pt1,'delimiter',',','-append');
            
        end
        
		%Convert ortho tables to skew tables if zeta=0
        if zeta==0
			outdir_ortho=strcat(outdir,'\ortho_to_skew');   
			
			if ~exist(outdir_ortho,'dir')
				mkdir(outdir_ortho);
			end
			...
			[qr,qt]=convert_tables_ortho_to_skew(pr,pt);
            [qrPP,qtPP]=convert_tables_ortho_to_skew(prPP,ptPP);
            [qrPS,qtPS]=convert_tables_ortho_to_skew(prPS,ptPS);
            [qrSP,qtSP]=convert_tables_ortho_to_skew(prSP,ptSP);
            [qrSS,qtSS]=convert_tables_ortho_to_skew(prSS,ptSS);
            
			
			for ii=1:length(retainord)
				ename=strcat('R_avg_',num2str(skex(ii)),'_',num2str(skey(ii)),'.csv');
				fname=strcat(parentdir,'\',outdir_ortho_ld,'\',ename);
				
				qr1=[thetas.' qr(:,:,ii)];	
				
				dlmwrite(fname,phis,'delimiter',',','coffset',1);
				dlmwrite(fname,qr1,'delimiter',',','-append');
                
                ename=strcat('R_PP_',num2str(skex(ii)),'_',num2str(skey(ii)),'.csv');
				fname=strcat(parentdir,'\',outdir_ortho_ld,'\',ename);
                ...
                ename=strcat('R_PS_',num2str(skex(ii)),'_',num2str(skey(ii)),'.csv');
				fname=strcat(parentdir,'\',outdir_ortho_ld,'\',ename);
				...
				dlmwrite(fname,qr1,'delimiter',',','-append');
                 ...
                 ename=strcat('R_SP_',num2str(skex(ii)),'_',num2str(skey(ii)),'.csv');
				fname=strcat(parentdir,'\',outdir_ortho_ld,'\',ename);
                ...
                ename=strcat('R_SS_',num2str(skex(ii)),'_',num2str(skey(ii)),'.csv');
				fname=strcat(parentdir,'\',outdir_ortho_ld,'\',ename);
        ...
        %save D05_nosym_orthotoskew_test_blue qr qt qrPP qrPS qrSP qrSS phis thetas MS Notch  retainord cx cy retnxy
	
	
		%Copy all files to R drive if rdrfold is not empty.
        %Make a copy of the raw config file as well.
		if ~isempty(rdrfold)
			source_folder=strcat(parentdir,'\',outdirld);
			dest_folder=strcat(rdrfold,'\',outdirld);
			copyfile(source_folder,dest_folder);
			source_file=strcat(parentdir,'\',outdir,'\',raw_file);
			dest_folder1=strcat(rdrfold,'\',outdir);
			copyfile(source_file,dest_folder1);
		end
end

end%rcwa_data_phi_theta

function [outdir,outdirld]=dir_names(ldnm,Tnm,MS,Notch,height_grating_nm,thickness_coating_nm,oprefix,osuffix,rdrfold,cross)
fMS=floor(MS);fNotch=floor(Notch);
...
switch length(Tnm)
	...
	case 1
		...
		if fMS==MS 
			if fNotch==Notch
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(MS),st1,'_',num2str(Notch),st2,'_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(MS),st1,'_',num2str(fNotch),'p5',st2,'_',num2str(thickness_coating_nm),'T');
			end
			
		else
			if fNotch==Notch
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(fMS),'p5',st1,'_',num2str(Notch),st2,'_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm),'D_',num2str(height_grating_nm),'H_',num2str(fMS),'p5',st1,'_',num2str(fNotch),'p5',st2,'_',num2str(thickness_coating_nm),'T');
			end
	...
	case 2
		...
		if fMS==MS 
			if fNotch==Notch
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(MS),st1,'_',num2str(Notch),st2,'_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(MS),st1,'_',num2str(fNotch),'p5',st2,'_',num2str(thickness_coating_nm),'T');
			end
			
		else
			if fNotch==Notch
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(fMS),'p5',st1,'_',num2str(Notch),st2,'_',num2str(thickness_coating_nm),'T');
				
			else
				outdir=strcat(num2str(Tnm(1)),'Dx_',num2str(Tnm(2)),'Dy_',num2str(height_grating_nm),'H_',num2str(fMS),'p5',st1,'_',num2str(fNotch),'p5',st2,'_',num2str(thickness_coating_nm),'T');
			end
...
outdir=strcat(oprefix,outdir,osuffix);
ldstr=num2str(ldnm);
outdirld=strcat(outdir,'\',ldstr);
...
phi_shift0=-120;
[aef pptr]=min(abs(phis-phi_shift0));
